...
interface LinkedInJobSearchPageProps {
  isDark?: boolean;
  toggleTheme?: () => void;
}

const LinkedInJobSearchPage = ({ isDark, toggleTheme }: LinkedInJobSearchPageProps) => (
  <ProjectPageTemplate
    isDark={isDark}
    toggleTheme={toggleTheme}
    title="LinkedIn Job Search (n8n)"
    description="A sophisticated low-code automation workflow engineered with n8n to revolutionize the job search process. This system autonomously monitors LinkedIn for varied tech roles, applying complex filtering logic to curate high-quality opportunities. It seamlessly integrates with Google Sheets for application tracking and delivers instant multi-channel notifications, ensuring users never miss a critical opening."
    ...
    features={[
      { id: "discovery", icon: <Activity className="w-4 sm:w-5 h-4 sm:h-5" />, title: "Automated Job Scraping Pipeline", summary: "High-frequency polling of LinkedIn for real-time job market data", details: ["Configurable HTTP Request nodes mimicking browser behavior to fetch raw job listings", "Robust HTML parsing (or API consumption) to extract key metadata like salary and tech stack", "Normalization of unstructured data fields into a consistent JSON schema for processing"] },
    ...
    useCases={[
      "Automated daily surveillance of LinkedIn for niche technical roles using custom search parameters",
    ...
    howToSteps={[
      ...
      <>Add credentials:&nbsp;Configure LinkedIn source (cookie/login), Google Sheets, and your alert integrations.</>,
      <>Set parameters:&nbsp;Define job titles, location IDs, and filtering logic (e.g., exclude "senior" or "intern").</>,
...
export default LinkedInJobSearchPage;
