            ...
            return (
              ...
              >
                <Link
                  ...
                  className="block rounded-3xl overflow-hidden border border-blue-200/60 dark:border-white/[0.08] bg-blue-50/30 dark:bg-slate-950/90 backdrop-blur-md hover:shadow-xl hover:shadow-blue-500/[0.08] dark:hover:shadow-sky-500/[0.05] transition-all duration-500 hover:border-blue-300/50 dark:hover:border-sky-400/20 hover:-translate-y-1 min-h-[44px] focus-override"
                >
                  {/* Responsive flex: row on mobile, column on sm+ */}
                  <div className="flex flex-row sm:flex-col h-full">
                    {/* Full-height image on the left for mobile, top for sm+ */}
                    <div className="relative w-1/3 h-auto sm:w-full sm:aspect-[16/10] flex-shrink-0 overflow-hidden rounded-l-2xl sm:rounded-t-3xl sm:rounded-l-none">
                      <img
                        src={project.image}
                        alt={project.title}
                        className="h-full w-full object-cover object-center scale-110 transition-transform duration-700 group-hover:scale-125 sm:scale-100 sm:group-hover:scale-105"
                        loading="lazy"
                      />
                      {/* Modern gradient overlay only on sm+ */}
                      <div className="hidden sm:block absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                      ...
                      <div className="hidden sm:block absolute inset-0 ring-1 ring-inset ring-black/[0.08] dark:ring-white/[0.08] rounded-t-3xl sm:rounded-t-3xl sm:rounded-b-none" />
                    </div>
                    {/* Content right on mobile, bottom on sm+ */}
