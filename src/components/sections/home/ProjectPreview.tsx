          ...
          {featuredProjects.map((project, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* Responsive flex: row on mobile, column on sm+ */}
                  <div className="relative flex flex-row sm:flex-col h-full">
                    {/* Project image: only visible on desktop (sm+) */}
                    <div className="hidden sm:static sm:block w-full sm:aspect-[16/10] flex-shrink-0 overflow-hidden rounded-3xl sm:rounded-t-3xl sm:rounded-l-none z-0">
                      <img
                        src={project.image}
                        alt={project.title}
                        className="h-full w-full object-cover object-center scale-110 transition-transform duration-700 group-hover:scale-125 sm:scale-100 sm:group-hover:scale-105 rounded-t-3xl"
                        loading="lazy"
                      />
                      {/* Modern gradient overlay only on sm+ */}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                      {/* Category badge with icon only on sm+ */}
                      ...
                      <div className="absolute inset-0 ring-1 ring-inset ring-black/[0.08] dark:ring-white/[0.08] rounded-t-3xl sm:rounded-t-3xl sm:rounded-b-none" />
                    </div>
                    {/* Content always on top, with relative z-index */}
                    <div className="relative z-10 flex-1 p-4 sm:p-5 lg:p-6 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-200 leading-relaxed mb-3 sm:mb-4 min-h-[32px] sm:min-h-[40px]">
                      ...
                      <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-auto">
                        {project.tags.slice(0, 2).map((tag) => (
                          <span
                            key={tag.name}
                            className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium rounded-xl border border-slate-200/70 dark:border-white/[0.08] text-slate-700 dark:text-slate-200 bg-slate-50/80 dark:bg-slate-800/50 backdrop-blur-sm hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                          >
                        ...
                        {project.tags.length > 2 && (
                          <span className="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium rounded-xl border border-blue-200/70 dark:border-sky-400/20 text-blue-700 dark:text-sky-200 bg-blue-50/80 dark:bg-sky-900/20 backdrop-blur-sm">
                            +{project.tags.length - 2}
                          </span>
                        )}
                      </div>
